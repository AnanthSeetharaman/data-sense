...
const SIDEBAR_COOKIE_NAME = "sidebar_state" // For collapsed/expanded state
const SIDEBAR_WIDTH_STORAGE_KEY = "sidebar_width" // For resized width
const SIDEBAR_COOKIE_MAX_AGE = 60 * 60 * 24 * 7
const DEFAULT_SIDEBAR_WIDTH = "16rem"
const SIDEBAR_WIDTH_MOBILE = "18rem"
const SIDEBAR_WIDTH_ICON = "3rem"
const MIN_SIDEBAR_WIDTH_PX = 180 // Minimum width in pixels
const MAX_SIDEBAR_WIDTH_PX = 500 // Maximum width in pixels
...
>(
  (
    ...
    React.useEffect(() => {
      ...
      if (storedWidth) {
        setSidebarWidthState(storedWidth)
      }
      const collapsedState = document.cookie
        .split("; ")
        .find((row) => row.startsWith(`${SIDEBAR_COOKIE_NAME}=`))
        ?.split("=")[1]
      if (collapsedState) {
        _setOpen(collapsedState === "true")
      }
    }, [])
      ...
      <SidebarContext.Provider value={contextValue}>
        <TooltipProvider delayDuration={0}>
          <div
            style={
              {
                "--sidebar-width": state === "expanded" ? sidebarWidth : SIDEBAR_WIDTH_ICON,
      ...
      >
        <div
          className={cn(
            ...
            isResizing ? "transition-none" : "duration-200 transition-[width]",
        ...
        <div
          className={cn(
            "fixed inset-y-0 z-10 hidden h-svh md:flex ease-linear",
            isResizing ? "transition-none" : "duration-200 transition-[left,right,width]",
            side === "left"
